...
# not $CLEMENTINE_CLI
# CLI="./target/debug/clementine-cli"
...
function withdraw_payout() {
    ...
    # 10BTC - 330sats - operator_withdrawal_fee_sats(0.001BTC)
    WITHDRAWAL_AMOUNT=999899670
...
function optimistic_withdraw_cli() {
    if [[ ! -f $WITHDRAW_UTXO_FILE ]]; then
        echo "❌️ File not found: $WITHDRAW_UTXO_FILE"
        exit 1
    fi
...
# Vault UTXOからユーザに支払う
function start_optimistic_withdraw() {
